...
if (isset($_GET['id'])) { // получен id запрошенной категории, который проверяется и сохраняется
    $_SESSION['category_id'] = mysqli_real_escape_string($con, $_GET['id']);
...
$page_content = include_template('all-lots.php', ['categories'=> $categories, 'lots' =>$lots,  'h2_text' => $h2_text,  'time_to_end' => $time_to_end,
                                        'pages_count' => $pages_count, 'cur_page' => $cur_page, 'pages' => $pages]);
$layout_content = include_template('layout.php', ['content' => $page_content, 'categories'=> $categories,'h2_text' => $h2_text,
                                       'title' => 'YetiCave - Результаты поиска по категории','user_name' => $user_name]);
