...
$sql = "SELECT c.name category, l.id, l.image, l.description, l.date_create, l.current_price, l.date_finish,
		        l.name, l.page_adress, l.start_price, l.step_rate, l.user_id, l.count_rates, l.user_winner_id FROM lots l JOIN categories c ON l.category_id = c.id
				WHERE l.id = $lot_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $lot_rate_new  = mysqli_real_escape_string($con, $_POST['cost']);
        
    // валидация введенного значения
    if (empty($lot_rate_new)) {
        $errors['cost'] = 'Это поле нужно заполнить';
    } elseif (!is_numeric($lot_rate_new)  or !filter_var($lot_rate_new, FILTER_VALIDATE_INT)  or ($lot_rate_new  <  $min_price)) {
...
$page_content = include_template('lot.php', [ 'val' => $val , 'errors' => $errors, 'time_to_finish' => $time_to_finish, 'lot_rates' => $lot_rates,
                                       'add_ruble' => $add_ruble, 'categories'=> $categories, 'current_price' => $current_price, 'min_price' => $min_price]);
